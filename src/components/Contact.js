...
const ContactCard = ({ info }) => {
  ...
  return (
    ...
    >
      ...
      {/* Text */}
      <div className="flex flex-col items-center text-center gap-1.5 md:gap-3 w-full">
...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      /* 
        IMPORTANT: Go to https://formspree.io/, create a form, 
        and replace the URL below with your unique Formspree URL.
      */
      const FORMSPREE_URL = "https://formspree.io/f/xojygpdb"; 
      ...
      if (response.ok) {
        ...
        setStatus("error");
        alert("Please set your Formspree ID in Contact.js to receive emails!");
      }
    } catch (error) {
      setStatus("error");
  ...
  const contactData = [
    {
      label: "Email Me",
      value: "mdshantosharker@gmail.com",
      color: "#EA4335",
      link: "mailto:mdshantosharker@gmail.com",
      svgPath:
  ...
  return (
    <section className="py-20 md:py-28 bg-white relative overflow-hidden" id="contact">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="max-w-5xl mx-auto bg-white/70 backdrop-blur-3xl border border-slate-100 rounded-[3rem] p-8 md:p-20 shadow-2xl relative overflow-hidden">
          {status === "success" && (
            ...
            >
              <div className="w-20 h-20 bg-primary-container rounded-full flex items-center justify-center mb-6 shadow-2xl shadow-primary-container/20">
                <svg viewBox="0 0 24 24" className="w-10 h-10 text-white fill-current">
                  <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" />
                </svg>
              </div>
              <h3 className="text-3xl md:text-4xl font-black text-slate-900 mb-2">Message Sent!</h3>
              <p className="text-slate-500 font-medium max-w-xs">Thank you for reaching out. I'll get back to you as soon as possible.</p>
              <button 
                onClick={() => setStatus("idle")}
                className="mt-8 text-primary-container font-black uppercase tracking-widest text-xs hover:underline"
              >
          ...
          <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
            ...
            <button 
              ...
              className="group w-full h-16 md:h-20 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-[0.3em] hover:bg-primary-container transition-all relative overflow-hidden disabled:opacity-70 disabled:cursor-not-allowed"
            >
              <span className={`relative z-10 ${status === "sending" ? "opacity-0" : "opacity-100"}`}>
                Send Message
              </span>
              {status === "sending" && (
